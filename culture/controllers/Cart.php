...
$query_principal="SELECT magasinier.ID_MAGASINIER, concat(magasinier.NOM,' ',magasinier.PRENOM) AS NOM FROM fonction_magasinier LEFT JOIN magasinier ON magasinier.ID_MAGASINIER=fonction_magasinier.ID_MAGASINIER LEFT JOIN fonction ON fonction.ID_FONCTION=fonction_magasinier.ID_FONCTION WHERE 1 GROUP BY NOM";
...
$data['magasinier']=$this->Model->getRequeteOne("SELECT  concat(NOM,'',PRENOM) AS NOM FROM fonction_magasinier LEFT JOIN magasinier on magasinier.ID_MAGASINIER=fonction_magasinier.ID_MAGASINIER ");
 ...
 function detail($id)
   {
      $query_principal= 'SELECT fonction_magasinier.ID_FONCTION_MAGASINIER, fonction.DESCR_FONCTION AS descr,fonction.ID_FONCTION,fonction_magasinier    .ID_MAGASINIER FROM fonction LEFT JOIN fonction_magasinier ON fonction.ID_FONCTION=fonction_magasinier.ID_FONCTION WHERE fonction_magasinier.ID_FONCTION='.$id ;

      $var_search = !empty($_POST['search']['value']) ? $_POST['search']['value'] : null;
      $critaire='';

      $search = !empty($_POST['search']['value']) ? (" AND (fonction.DESCR_FONCTION LIKE '%$var_search%') ") :'';
      ...
      foreach($fetch_cov_frais as $info)
      {
        ...
        $post[]=$u++; 
        $post[]=$info->descr;
        $options="<a class='btn btn-danger btn-md' href='" . base_url('culture/Cart/del_occ/').$info->ID_FONCTION_MAGASINIER . "'>Supprimer</a>";
        $post[]=$options;
        $data[]=$post;
  ...
  public function del_occ($id){
   $this->Model->delete('occupation_acteur',array('ID_OCCUPATION_ACTEUR'=>$id));
   redirect(base_url('formation/Panier/'));
}
 ...
 function editerCart (){
  $id=$this->input->post('id');
      $achats=$this->Model->getRequete('SELECT ID_FONCTION_MAGASINIER, ID_FONCTION, ID_MAGASINIER FROM fonction_magasinier WHERE ID_MAGASINIER='.$id);

  // print_r($achats);die();

  foreach ($achats as $key) {
     // code...
   $file_data=array('id'=>uniqid(),'qty'=>1,
    'price'=>1,
    'name'=>'T',
    'ID_FONCTION'=>$key['ID_FONCTION'],'typecartitem'=>'FILE' );
   $this->cart->insert($file_data);
