...
{     		
	...
	function listing()
	{
		...
		foreach ($fetch_res as $row)
		{
			...
			$action .="<li>
			<a href='".base_url('culture/Publication/getOne/').$row->ID_PUBLICATION ."'>
			...
			<center>
			<h5><strong>Voulez-vous supprimé la publication?</strong><br> <b style:'background-color:prink';><i style='color:green;'>" . $row->ID_PUBLICATION."</i></b>
			</h5>
			</center>
			</div>

			<div class='modal-footer'>
			<a class='btn btn-danger btn-md' href='" . base_url('culture/Publication/delete/').$row->ID_PUBLICATION. "'>
			Supprimer
			</a>
	...
	function save()
	{
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			// print_r($data);die();
			$this->load->view('Add_Publication_View');
